...
public class MainRenderer implements GLSurfaceView.Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    public void onDrawFrame ( GL10 unused ) {
        //Log.i("TLD", "onDrawFrame");

        if ( !mGLInit ) return;
        GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);

        synchronized(this) {
            //Log.i("TLD", "synchronize");
            if ( mUpdateST ) {
                mSTexture.updateTexImage();
                if(mSTexture == null) {
                Log.i("TLD", "mSTexture null");
                }else{
                    Log.i("TLD", "mSTexture OK");
                }
                mUpdateST = false;
            }
    ...
    public synchronized void onFrameAvailable ( SurfaceTexture st ) {
        Log.i("TLD", "frameAviable");
        mUpdateST = true;
        mView.requestRender();
    }

    private static int loadShader ( String vss, String fss ) {
        Log.i("TLD", "loadShader");
    ...
    void openCamera() {
        ...
        try {
            ...
            Log.i("TLD", "OpenCamera - Camera Access Exception");
        } catch ( IllegalArgumentException e ) {
            Log.i("TLD", "OpenCamera - Illegal Argument Exception");
        } catch ( SecurityException e ) {
            Log.i("TLD", "OpenCamera - Security Exception");
        } catch ( InterruptedException e ) {
            Log.i("TLD", "OpenCamera - Interrupted Exception");
        }
    }
    ...
    private void createCameraPreviewSession() {
        Log.i("TLD", "createCameraPreview");
        try {
            Log.i("TLD", "tryCreateCameraPreview");
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(surface),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigured(CameraCaptureSession cameraCaptureSession) {
                            Log.i("TLD", "OnConfigured");
                            if (null == mCameraDevice) {
                                Log.i("TLD", "Camera null");
                                return;
                            }
                            ...
                            try {
                                ...
                                Log.i("TLD", "tryConfigured");
                                mCaptureSession.setRepeatingRequest(mPreviewRequestBuilder.build(), null, mBackgroundHandler);
                            } catch (CameraAccessException e) {
                                Log.i("TLD", "createCaptureSession");
                            }
                        }
                        @Override
    ...
    private void startBackgroundThread() {
        ...
        Log.i("TLD", "startBackgroundThread");
